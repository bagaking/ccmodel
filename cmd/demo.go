...
func runDemo(cmd *cobra.Command, args []string) error {
	...
	descriptions := []string{
		"Animated futuristic loading spinners",
		"Visualize neural network layer activations",
		"Showcase quantum-style progress bars",
		"Matrix-style data streaming effect",
		"Various loading animation styles",
		"All effects in one spectacular show",
		"Return to main menu",
	}

	title := "ccmodel Out-of-Box Demo"
	for {
		fmt.Print("\033[2J\033[H") // Clear screen
		selected, err := ux.InteractiveMenuWithDesc(title, options, descriptions)
		if err != nil || selected == 6 {
			fmt.Println("🚀 Exiting out-of-box demo...")
			return nil
		}
